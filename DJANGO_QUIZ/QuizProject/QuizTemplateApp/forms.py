...
from .models import CustomUser, UserProfile, Quiz, Question, Choice, Challenge
...
# Keep your existing custom forms
...
class ChallengeForm(forms.ModelForm):
    receiver_username = forms.CharField(max_length=100, help_text="Enter the username of the person you want to challenge")
    
    class Meta:
        model = Challenge
        fields = ['quiz', 'message', 'receiver_username']
        widgets = {
            'message': forms.Textarea(attrs={'rows': 3, 'placeholder': 'Add a message with your challenge'}),
        }
    
    def __init__(self, *args, sender=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.sender = sender
        # Show only public quizzes or quizzes created by the sender
        if sender:
            self.fields['quiz'].queryset = Quiz.objects.filter(
                models.Q(is_public=True) | models.Q(created_by=sender)
            )
    ...
    def save(self, commit=True):
        ...
        instance.receiver = CustomUser.objects.get(username=receiver_username)
